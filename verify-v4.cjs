...
function boot(initial={}){
  ...
  const api=`globalThis.test={get state(){return state},get session(){return session},get filters(){return missionFilters},get page(){return page},setPage(p){page=p},setFilters(f){missionFilters=f},reset(){state={attempts:[],runs:[],active:null};session=null;missionFilters={term:'',level:'all'}},abandon(){session=null;state.active=null},get features(){return features},get profiles(){return profileManager},reloadProfile(){session=null;state=profileManager.load()},get student(){return student},get interactive(){return interactive},Interactive,ACTIVE_TASKS,QUEST_WORDS,StudentModel,EQ,PRACTICE,studyDays,stats,streak,start,answer,next,resume,leaveSession,render,save,matchingQuestions,cards,updateMissions,dateKey,dayAgo,QUESTIONS,TOPICS,LEVELS};`;
