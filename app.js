...
let missionFilters={term:'',level:'all'};
try{
...
function start(mode){
  ...
  const custom=Interactive.plan(mode,state,dateKey())||EventQuest.plan(mode,state,dateKey())||student.customMode(mode)||features.customMode(mode);
...
document.addEventListener('change',e=>{if(e.target.id==='difficulty'){missionFilters.level=LEVELS.includes(e.target.value)?e.target.value:'all';updateMissions();}else if(!interactive.change(e)&&!student.change(e))features.change(e);});
